...
Procedura:
1. Crea bucket `nebula-backup` su Cloudflare R2 dashboard
2. Crea API token R2 con scope: Object Read & Write, bucket: `nebula-backup`
3. Popola `secrets/rclone-env.enc.yaml` con le credenziali
4. Cifra: `sops --encrypt --in-place secrets/rclone-env.enc.yaml`
5. `nixos-rebuild switch --flake .#nebula` (il secret viene montato in
...
rclone ls r2:nebula-backup/technitium/
rclone ls r2:nebula-backup/k3s/
rclone size r2:nebula-backup/
...
### Rebuild completo (scenario: nebula perso o disco cambiato)
...
3. `nixos-rebuild switch --flake .#nebula` per attivare moduli (Technitium,
   k3s, backup, ecc.)
...
rclone delete r2:nebula-backup/ --min-age 30d --include "**"
```

Da aggiungere a un timer separato se il bucket cresce troppo. Free tier R2
è 10 GB, sufficiente per anni di backup di Nebula.

## DR test (da fare)

Pianificare almeno una volta dopo la migrazione:
1. Distruggi e ricrea `nebula` da zero seguendo questo doc
