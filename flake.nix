# Houston — flake NixOS
#
# Entry point per la configurazione NixOS di houston.
# `nixosConfigurations.houston` è il sistema principale (server Dell Optiplex 3050).
#
...
#   nixos-rebuild switch --flake .#houston --target-host root@192.168.178.2 --build-host localhost
#
# Per installare/reinstallare via SSH (richiede Linux già avviato sul target, es. Debian live):
#   nix run github:nix-community/nixos-anywhere -- --flake .#houston root@192.168.178.2
#
# Pinning:
...
{
  description = "Houston — NixOS homelab su Dell Optiplex 3050";
  ...
  inputs = {
    ...
    # Uso: nix run .#nixos-anywhere -- --flake .#houston root@192.168.178.2
    nixos-anywhere = {
  ...
  outputs = { self, nixpkgs, sops-nix, disko, nixos-anywhere, impermanence, nixos-hardware, ... }: {
    nixosConfigurations.houston = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
